    ...
    <div className="border p-2 rounded">
      {selectedFiles.length === 0 ? (
        <label className="border-2 rounded p-6 text-center bg-[#F5F8F8] cursor-pointer h-[222px] flex flex-col items-center justify-center">
          {selectedImage && selectedImage ? <img className="w-full h-full object-cover rounded" src={`${selectedImage.image_name?selectedImage.image_name:selectedImage}`} alt={selectedImage.alt_text} /> :
           <>   {image!=null ? (
            <img className="w-full h-full object-cover rounded" src={`${image}`} alt={altText} />
          ) : (
            <>

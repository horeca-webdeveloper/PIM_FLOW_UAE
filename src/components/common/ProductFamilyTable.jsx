...
const ProductFamilyTable = ({
  ...
  const processedData = !!datas && datas?.map(item => ({
    ...item,
    total_arr: item.attribute_groups?.length ?? 0
  }));
    ...
    <>
      ...
      <div className="p-0 bg-white border-t border-l border-r border-[#BCBCBC] rounded-md mt-[20px] capitalize">
        ...
        >
          ...
          <Column
            field="total_arr"
            header="Total Attribute"
            body={(rowData) => rowData.total_arr}
            sortable
            filter
            bodyClassName={cellClass}
            headerClassName={headerClass}

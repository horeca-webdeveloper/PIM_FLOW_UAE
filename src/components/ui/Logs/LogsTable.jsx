...
const LogsTable = ({ tableHeading, datas, showFilter, showCheckBox, getDatafn, deleteData, disableDelete, disableEdit, disableView, setPage, changePage, currentPage,totalPages,totalRecords,fetchErrorLogs}) => {
                                    ...
                                    {tableHeading.map((column, colIndex) => (
                                        ...
                                        >
                                            {column.key === "created_by"
                                                ? `${row[column.key]?.first_name || ""} ${row[column.key]?.last_name || ""}`
                                                : row[column.key]}
                                        </td>
                                    ))}
                                    <td className="p-3 items-center">
                                        <div className="flex  justify-center space-x-3 w-[100px] p-1 border rounded-md border-[#BCBCBC]">
                                            {!disableEdit ? <span className="text-blue-500 hover:text-blue-700 cursor-pointer" onClick={() => getDatafn(row.id)}>
                                                <img src={`${urls.hostUrl}/icons/pencil-write.png`} alt="edit" />
                                            </span> : ''}

                                            {!disableView ? <span onClick={()=>fetchErrorLogs(row.id)} className="text-blue-500 hover:text-blue-700 cursor-pointer">
                                                <img src={`${urls.hostUrl}/icons/view.png`} alt="edit" />
                                            </span> : ''}

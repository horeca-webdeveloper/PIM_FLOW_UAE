...
const BrandCustomizationThird = () => {
  ...
  const onSubmit = (data) => {
    ...
    const categoryIds = Object.entries(grouped).map(([category_id, product_ids]) => ({
    ...
    if (uid) {
      ...
      Apis.updateThirdBrandPage(formData, uid, setLoader, setResponse);
    } else {
      Apis.createThirdBrandPage(formData, setLoader, setResponse);
    }
  };
  ...
  useEffect(() => {
    if (uid) {
      Apis.fetchBrandThirdPageById(uid, setLoader, setBrandPageDetails);
  ...
  useEffect(() => {
    if (uid && brandPageDetails && categories?.categories) {
      ...
      Apis.filterCategoryByCategoryIdThird(categoryIdsString, setCategoryLoader, setFilterCategory);

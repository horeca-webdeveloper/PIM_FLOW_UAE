...
const BrandListFirst = () => {
  ...
  const deleteBrandById = (id) => {
    ...
    if (confirmDelete) {
      Apis.deleteBrandFirst(id, setLoader, setResponse);
    }
  };

  useEffect(() => {
    Apis.fetchBrands(setBrandLoader, setBrands);
    Apis.fetchBrandFirstPage(setBrandLoader, setBrandData);
  }, []);
        ...
        <div>
          <form onSubmit={handleSubmit(onSubmit)}>
            <HeaderComponent
              ...
              setShow={setShow}
              span={`(${
                !!getBrandDatas && getBrandDatas?.data?.length
              } Results)`}
              buttons={buttons}
            />
            ...
            <CreateBrandsPopup
            title="Create First Brand Template"
              loader={loader}
              control={control}
...
export default React.memo(BrandListFirst);

...
const UpdateSecondVendorValuationPopup = ({
  setShowEdit,
  setSecondShowEdit,
  updateId,
  updateData,
  ...
  console.log(fetchData?.product_demand_level);
  ...
  useEffect(() => {
    if (
      fetchData?.product_demand_level &&
      fetchData.product_demand_level.length > 0
    ) {
      const updatedForms = fetchData.product_demand_level.map(
        (item, index) => ({
          id: index + 1,
          ...
          competitor_price_online: item.competitor_price_online || "",
          competitor_price_offline: item.competitor_price_offline || "",
          margin_auto_calculate: item.margin_auto_calculate || "",
        })
      );
      ...
      // Reset RHF values
      const resetValues = {
        product_demand_level: fetchData.product_demand_level.map((item) => ({
          product_name: item.product_name || "",
          primary_keyword: item.primary_keyword || "",
          search_volume: item.search_volume || "",
          supplier_price: item.supplier_price || "",
      ...
      reset(resetValues); // 🛠 Resetting RHF with prefilled data
    }
  }, [fetchData]);
  ...
  const onSubmit = async (payload) => {
    ...
    const combineData = { ...updateData, ...payload };
    console.log(combineData);
    const { id, ...data } = combineData;
    setLoader(true);
    const token = localStorage.getItem("token");
    try {
      const response = await axios.put(
        `${baseUrls}/pre-onboarding-vendors/${updateId}`,
        data,
        {
          headers: {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-[30%] p-3 relative max-h-[90vh] ">
        <button
        ...
        <h2 className="text-xl font-semibold mb-1">Update Vendor Valuation</h2>
...
export default UpdateSecondVendorValuationPopup;
